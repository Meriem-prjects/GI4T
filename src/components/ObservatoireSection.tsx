...
const ObservatoireSection = () => {
  ...
  return <Card role="button" tabIndex={0} aria-label="Accéder à l'Observatoire des Droits" onClick={() => navigate('/observatoire')} onKeyDown={e => {
    if (e.key === 'Enter' || e.key === ' ') {
      ...
      <CardContent className="p-8 sm:p-12 flex flex-col items-center justify-center space-y-8 min-h-[500px]">
        {/* Logo */}
        <div className="flex-shrink-0 my-[15px] mx-0">
          <img src={language === 'ar' ? logoOdfAr : logoOdfFr} alt="Observatoire des Droits Fondamentaux" className="h-32 sm:h-40 md:h-48 w-auto object-contain" />
        </div>
        ...
        {/* Search Input */}
        <div className="w-full max-w-md relative z-10" onClick={e => e.stopPropagation()} onMouseDown={e => e.stopPropagation()} onKeyDown={e => e.stopPropagation()} onTouchStart={e => e.stopPropagation()}>
          <SearchAutocomplete value={searchQuery} onChange={setSearchQuery} onSearch={handleSearch} placeholder={animatedPlaceholder} language={language} />
        </div>
      </CardContent>
    </Card>;
};
