...
/**
 * Remove keywords lines from content (both Arabic and French)
 * Filters out lines containing "الكلمات المفاتيح" or "Mots-clés" / "Mots clés"
 */
export const removeKeywordsFromContent = (content: string): string => {
  if (!content) return '';
  
  const lines = content.split('\n');
  return lines.filter(line => {
    const cleanLine = line.replace(/<[^>]*>/g, '').toLowerCase();
    return !cleanLine.includes('الكلمات المفاتيح') && 
           !cleanLine.includes('mots-clés') &&
           !cleanLine.includes('mots clés') &&
           !cleanLine.includes('الكلمات المفتاحية');
  }).join('\n');
};

/**
 * Remove repeated title from beginning of content
 * Filters out lines in the first 10 lines that match the document title
 */
export const removeTitleFromContent = (content: string, title: string): string => {
  ...
  if (cleanTitle.length < 5) return content;
  
  const filteredLines = lines.filter((line, index) => {
    // Only check first 10 lines for title matches
    if (index > 10) return true;
    
    const cleanLine = line.replace(/<[^>]*>/g, '').trim().toLowerCase();
    
    // Skip empty lines
    if (cleanLine.length < 3) return true;
    
    // Check if line contains title or title contains line (for partial matches)
    const titleMatch = cleanLine.includes(cleanTitle) || cleanTitle.includes(cleanLine);
    
    // Only filter if it's a strong match (similar length)
    if (titleMatch && Math.abs(cleanLine.length - cleanTitle.length) < 20) {
      return false;
    }
